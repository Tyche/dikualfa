...
void do_set (struct char_data *ch, char *argument, int cmd)
{
  ...
  /* from spell_parser.c */
  char *spells[] = {
    "armor", "teleport", "bless", "blindness", "burning hands",
      "call lightning",
  ...
  if (str_cmp (buf, "skill") && str_cmp (buf, "s") && str_cmp (buf, "value")
    && str_cmp (buf, "v")) {
    send_to_char ("'skill' or 'value' expected.\n\r", ch);
    return;
  }
    ...
    /* This is a skill */
  {
    ...
    sprintf (buf2, "%s changes %s's %s to %d,%s.", GET_NAME (ch),
      GET_NAME (vict), spells[skill], value, buf);
    ...
    sprintf (buf2, "%s sets %s's %s to %s.", GET_NAME (ch), GET_NAME (vict),
      values[skill], buf);
    switch (skill) {
    case 0:                    /* age */
      {
    ...
    case 2:                    /* class */
      {
        ...
        switch (*buf) {
        case 'm':
          vict->player.class = CLASS_MAGIC_USER;
          break;
        case 'c':
          vict->player.class = CLASS_CLERIC;
          break;
        case 'w':
          vict->player.class = CLASS_WARRIOR;
          break;
        case 't':
          vict->player.class = CLASS_THIEF;
          break;
        }
      }
      break;
